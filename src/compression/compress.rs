...
mod tests {
    ...
    pub fn split_moves_by_comma(joined_moves: &str) -> Result<Vec<Move>, ChessError> {
        let move_strs: Vec<String> = joined_moves.split(',').filter(|it| it.len() > 0).map(|it| it.trim().to_string()).collect();
        let mut moves = Vec::new();
        for move_str in move_strs {
            moves.push(move_str.parse::<Move>()?);
        }
        Ok(moves)
    }
    ...
    fn test_get_fen_part1(
        ...
        let moves: Vec<Move> = split_moves_by_comma(joined_moves).unwrap();
